...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        # Fixme: Added HR-LR images down-up scale factor, sr
        self.factor = self.args.factor
        self.sr = self.args.sr
    ...
    def _setup_train(self, world_size):
        ...
        # Fixme: added HR image size
        self.args.hr_imgsz = check_imgsz(self.args.hr_imgsz, stride=gs, floor=gs, max_dim=1)
    ...
    def _do_train(self, world_size=1):
        ...
        #Emissions Tracker
        tracker = EmissionsTracker(save_to_file = True, output_dir = str(self.args.project)+"/"+str(self.args.name))
        tracker.start()
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    # Fixme: added SR loss computation
                    if self.args.sr:
                        if self.args.focal_sr: 
                            (self.loss, self.loss_items), self.sr_loss = self.model(batch)
                            self.loss += self.sr_loss             
                               
                        else: 
                            (self.loss, self.loss_items), self.sr_loss = self.model(batch)
                            #DEBUG
                            print(f"loss yolo:  {self.loss}")
                            print(f"loss super resolution:  {self.sr_loss}")  
                            self.loss += self.sr_loss
                    else:
                    ...
                    if self.args.sr:
                        self.tloss = (self.tloss * i + torch.cat((self.loss_items, torch.unsqueeze(self.sr_loss.detach(), 0)))) / (i + 1) if self.tloss is not None \
                            else torch.cat((self.loss_items, torch.unsqueeze(self.sr_loss.detach(), 0)))
                    else:
