...
def validate_request_params(request):
    ...
    data = request.get_json()  # Ottiene il JSON inviato alla richiesta
    ...
    if not dataset:
        return {'error': jsonify({'error': 'Dataset not found'}), 'status_code': 404}
    ...
    if model_category is None:
        return {'error': jsonify({'error': 'Invalid model_id'}), 'status_code': 400}

    # Check if the provided model_version is valid 
    model = model_category.get(model_version)
    if model is None:
        return {'error': jsonify({'error': 'Invalid model_version'}), 'status_code': 400}
