    ...
    def preprocess_batch(self, batch):
        ...
        if self.sr and (self.factor > 1):
            ...
            if self.args.focal_sr:
                ...
                #DEBUG 
                print(f"np.log(w*h/np.sum(out)):  {np.log(w*h/np.sum(out))}")

                # Apply mask weights' logarithmic smoothing ! ALTERNATIVE TO  no smoothing (correzione prof)
                out = out * np.log((w*h / (np.sum(out, axis=(1, 2))[:, np.newaxis, np.newaxis] + 1e-10))) + 1
                # Apply mask weights' no smoothing ! ALTERNATIVE TO  logarithmic smoothing
                # out = out * (w*h / (np.sum(out, axis=(1, 2))[:, np.newaxis, np.newaxis] + 1e-10)) + 1

                # Apply mask weights' logarithmic smoothing                #questo è vecchio smoothing
                ...
                #plt.imshow(out[29], cmap='gray')
                #plt.show()
                #plt.imshow(batch['hr_img'][29].cpu().permute(1, 2, 0))
                #plt.show()
        else:
